...
(function() {
  ...
  var A, B, C, DestructibleEntity, E, Entity, EphemeralEntity, H, MovingEntity, NavPointEntity, PlayerShipEntity, RockEntity, ShipEntity, TransportShipEntity, baseByFaction, setPosByName,
  ...
  ShipEntity = (function(superClass) {
    ...
    ShipEntity.prototype.update = function(dt) {
      ...
      if (this.warping) {
        this.warping += dt;
        this.vel.scale(1 - C.shipDockingDrag * dt);
        if (this.warping > C.shipWarpingTime) {
          this.warped = true;
        }
      }
    ...
    ShipEntity.prototype.beginWarping = function() {
      if (this.canWarp) {
        return this.warping += 1;
      } else {
        return this.warping = 0;
      }
    };
  ...
  setPosByName = function(pos, name, base, start) {
    var r;
    if (base == null) {
      base = true;
    }
    ...
    if (base) {
      pos.transXY(C.baseLocations[name][0], C.baseLocations[name][1]);
    } else {
      pos.transXY(C.navPtLocations[name][0], C.navPtLocations[name][1]);
    }
    return pos;
  };
  ...
  E.newRandomTransport = function(faction, inbound) {
    var a, base, nav, start, stop, transport, v, vel;
    if (inbound == null) {
      inbound = false;
    }
  ...
  E.PlayerShip = function() {
    var playerShip;
    playerShip = new PlayerShipEntity("ray", "civ", H.pt1.setXY(C.navPtLocations["Alpha Octolindis"][0], C.navPtLocations["Alpha Octolindis"][1]), H.HALFPI, H.pt2.setXY(0, C.shipInitialVeloctity), 0);
    return playerShip;
  };

...
(function() {
  ...
  var A, C, H, M, S, State, U, changeState, currentState, drawText, eng, getCtx, getModel, getPlayer, getState;
  ...
  S.gameOver = {
    ...
    draw: function(ctx) {
      ...
      return drawText(ctx, "Press [Escape] to restart.", 15, ctx.canvas.width / 2, ctx.canvas.height / 2 + 130);
    },
    update: function(dt) {
  ...
  S.dockMode = {
    ...
    draw: function(ctx) {
      S.play.draw(ctx);
      this.msg.draw(ctx);
      drawText(ctx, "You are docked.", 30, ctx.canvas.width / 2, ctx.canvas.height / 2 - 200);
      return drawText(ctx, "Press [Escape] to return.", 15, ctx.canvas.width / 2, ctx.canvas.height / 2 + 130);
    },
    update: function(dt) {},
    input: function(type, data) {
      if (type === "keydown") {
